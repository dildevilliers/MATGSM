...
% Reliable: 1
% FIXED VERSION OF THE CODE IN THE MAAT TOOLBOX DESCRIBED ABOVE
% edited: Dirk de Villiers, 2022/01
%--------------------------------------------------------------------------
if (nargin==3)
   Direction = 'h';
elseif (nargin==4)
...
if (Direction=='h')
   ...
   SinAz  = (-cos(Dec).*sin(HA))./(CosAlt+eps);
   CosAz  = (sin(Dec).*cos(Lat) - cos(Dec).*cos(HA).*sin(Lat))./(CosAlt + eps);
...
elseif (Direction=='e')
   ...
   SinHA  = (-cos(Alt).*sin(Az))./(CosDec+eps);
   CosHA  = (sin(Alt).*cos(Lat) - cos(Alt).*cos(Az).*sin(Lat))./(CosDec + eps);
